...
// jsonFieldSkip reports whether encoding/json would omit the field entirely
// because it is tagged json:"-" (exactly). Note json:"-," does NOT skip; it
// names the field "-".
func jsonFieldSkip(field reflect.StructField) bool {
...
// struct field, honoring the `json:"name"` tag. It assumes the field is not
// skipped (callers must consult jsonFieldSkip first). json:"-," names the
// field "-".
...
// (possibly a named type). Bytes() works for byte-element slices.
...
// key: string keys verbatim, other keys via their default text representation.
func mapKeyString(key reflect.Value) string {
	switch key.Kind() {
	case reflect.String:
		return key.String()
	default:
		// Fall back to fmt for integer keys (the common non-string case); this
		// keeps numeric map keys JSON-correct.
